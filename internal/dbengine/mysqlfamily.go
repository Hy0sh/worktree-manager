...
// mysqlFamily covers mysql and mariadb, which differ only by binary names and
// two variables. Credentials go through MYSQL_PWD, which both honour, so an
// empty password (MYSQL_ALLOW_EMPTY_PASSWORD) works and nothing is prompted.
...
// SECURITY: cmd is concatenated into a shell line, so only literals and
// TempDBName-sanitised identifiers may reach it, never config or user input.
func (e mysqlFamily) sh(cmd string) []string {
	return []string{"sh", "-c", "MYSQL_PWD=" + e.password + " exec " + cmd}
}

// ReadyArgs runs an authenticated query rather than a ping: during the image's
// first initialisation a temporary server answers pings while root's password
// is not set up yet, and only the final server accepts the query.
...
func (e mysqlFamily) RestoreScript(project string) string {
	return restoreScript(project, `echo "wtm: restoring $dump into `+strings.ReplaceAll(e.database, `"`, ``)+`"
